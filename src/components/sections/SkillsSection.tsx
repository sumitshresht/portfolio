...
const SkillBlock = ({ block, index }: { block: any, index: number }) => {
  ...
  return (
    <motion.div 
      ref={blockRef} 
      animate={isBlockInView ? "visible" : "hidden"} 
      variants={{ 
        hidden: { opacity: 0, scale: 0.98 }, 
        visible: { opacity: 1, scale: 1, transition: { duration: 0.8, ease: "easeOut" } } 
      }} 
      className="group relative bg-surface p-8 rounded-3xl shadow-neu-dark border border-white/5 hover:border-magic-glow/30 transition-all duration-700 overflow-hidden"
    >
      ...
      <div className="relative z-10">
        <h3 className="text-sm font-mono tracking-widest text-magic-glow/80 uppercase mb-6 flex items-center gap-2">
          <span className="w-2 h-2 border border-magic-glow bg-magic-glow/20 animate-pulse" />
          {block.category}
        </h3>
        
        <div className="flex flex-wrap gap-2.5 perspective-1000">
          {block.tech.map((t: string, idx: number) => {
            ...
            return (
              <motion.span 
                ...
                custom={idx} 
                variants={{ 
                  hidden: { opacity: 0, x: isLeft ? -100 - (idx * 40) : 100 + (idx * 40), y: isTop ? -100 - (idx * 30) : 100 + (idx * 30), z: -200, rotateX: isTop ? 45 : -45, rotateY: isLeft ? -45 : 45, rotateZ: isLeft ? -15 : 15, filter: "blur(8px)" }, 
                  visible: { opacity: 1, x: 0, y: 0, z: 0, rotateX: 0, rotateY: 0, rotateZ: 0, filter: "blur(0px)", transition: { type: "spring", stiffness: 80, damping: 14, mass: 1.2, delay: idx * 0.08 } } 
                }} 
                className="px-3 py-1.5 bg-canvas/90 backdrop-blur-xl rounded-lg text-[10px] font-mono tracking-widest uppercase text-gray-400 border border-white/10 shadow-[0_4px_10px_rgba(0,0,0,0.5)] group-hover:text-magic-glow group-hover:border-magic-glow/40 transition-colors relative" 
